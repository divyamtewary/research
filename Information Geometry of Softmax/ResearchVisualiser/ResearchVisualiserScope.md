...
| 3 | Partition function Z and log-normaliser A(λ) | Real-time Z calculator with animated sensitivity |
| 4 | Gradient of the log-normaliser ∇A = P | Visual proof that gradient components equal probabilities |
| 5 | Self-derivative ∂Pᵢ/∂λᵢ = Pᵢ(1−Pᵢ) | Sigmoid-like curve explorer |

## Scope — Part 2 (ResearchScope_02)

| Section | Concept | Interactive Element |
|---------|---------|-------------------|
| 1 | Cross-derivative ∂Pⱼ/∂λᵢ = −PᵢPⱼ | Coupled sliders showing token competition |
...
└── mathjax/
    └── tex-chtml.js              ← Local MathJax for offline use
```